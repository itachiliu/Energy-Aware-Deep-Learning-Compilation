...
\begin{itemize}[leftmargin=2em,itemsep=1pt]
  ...
  \item \textbf{S3, coding and agreement}: \texttt{coding/coding-121-final.csv},
    the four first-round sheets \texttt{coding-sheet-A/B-v2.csv} and
    \texttt{-v3.csv}, the two codebooks
    \texttt{codebook-*-worked-examples.csv}, the controlled vocabularies
    \texttt{coding-rubric-v2.md} and \texttt{-v3.md}, the disagreement list
    \texttt{reconciliation-sheet.csv}, the two reconciliation rounds
    \texttt{reconciliation-resolution-1-full.csv} and \texttt{-2.csv}, the
    final resolution table \texttt{reconciliation-resolution-final.txt}, the
    third-round adjudication package \texttt{adjudication-round3-*}, and the
    $\kappa$, merge and appendix-synchronisation scripts with their reports
    under \texttt{coding/kappa/};
  \item \textbf{S4, historical audit}: \texttt{audit/appendix-matrix-code.csv},
    \texttt{code\_matrix.py}, \texttt{internal-audit.md};
  ...
  \item \textbf{S6, small-scale validation experiment}: \texttt{experiment/},
    holding the protocol, the per-scan result tables, the environment
    snapshots, the job package and the plotting scripts; the per-run
    measurement logs are collected in
    \texttt{experiment/data/raw-measurement-logs.zip};
...
For the experiment reported in the small-scale validation experiment section, the measurement protocol, the raw per-run measurements, the configuration matrix, the job package and the plotting scripts are all under \texttt{experiment/}: the description is \texttt{experiment/README.md}, the environment and acceptance commands are \texttt{experiment/ENV\_AND\_EXPERIMENT.md}, the per-scan result tables and summaries are under \texttt{experiment/data/} together with the archived per-run logs, the job package is \texttt{experiment/job/}, and the plotting scripts are \texttt{experiment/scripts/}. The complete configuration matrix of the second batch is also given directly as \texttt{experiment/batch2\_5models\_results.csv}.
