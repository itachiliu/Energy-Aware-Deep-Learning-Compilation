...
\begin{itemize}[leftmargin=2em,itemsep=1pt]
  ...
  \item \textbf{S3 · 编码与一致性}：\texttt{coding/coding-121-final.csv}、
    四份第一轮独立编码表 \texttt{coding-sheet-A/B-v2.csv} 与 \texttt{-v3.csv}、
    两份示例 \texttt{codebook-*-worked-examples.csv}、受控词表
    \texttt{coding-rubric-v2.md} 与 \texttt{-v3.md}、争议清单
    \texttt{reconciliation-sheet.csv}、两轮复议记录
    \texttt{reconciliation-resolution-1-full.csv} 与 \texttt{-2.csv}、
    最终决议表 \texttt{reconciliation-resolution-final.txt}、第三轮裁定包
    \texttt{adjudication-round3-*}，以及 \texttt{coding/kappa/} 下的 $\kappa$、
    归并与附录同步脚本及其报告；
  ...
  \item \textbf{S6 · 最小验证实验}：\texttt{experiment/}（协议、各轮结果表、
    环境快照、作业包与绘图脚本）；逐次测量的原始日志汇总在
...
\section{S6 · 最小验证实验}
本文“小规模验证实验”一节报告的实验，其测量协议、逐次测量、配置矩阵、作业包与
绘图脚本均在 \texttt{experiment/} 下：说明见 \texttt{experiment/README.md}，
环境与验收命令见 \texttt{experiment/ENV\_AND\_EXPERIMENT.md}，各轮结果表与摘要
连同归档的逐次测量日志见 \texttt{experiment/data/}，作业包见
\texttt{experiment/job/}，绘图脚本见 \texttt{experiment/scripts/}。批次二的
完整配置矩阵另以 \texttt{experiment/batch2\_5models\_results.csv} 单独给出。
