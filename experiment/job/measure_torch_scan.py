# -*- coding: utf-8 -*-
"""PyTorch eager measurement with configurable batch (fp32/fp16).

Same power protocol as the mini-experiment; adds batch and keeps torchvision
random weights with a fixed seed. One inference unit is one batch call.
"""
import argparse
import csv
import os
...
def build_model(name, dtype, device):
    import torch
    import torchvision.models as M
    torch.manual_seed(0)
    model = getattr(M, TORCHVISION_SPECS[name])(weights=None).eval().to(device)
    if dtype == "fp16":
        model = model.half()
    return model
...
def main() -> int:
    ...
    ap.add_argument("--torch-name", required=True,
    ...
    with torch.no_grad():
        model = build_model(args.torch_name, args.dtype, device)
        x = torch.zeros(args.batch, 3, 224, 224, device=device)
        if args.dtype == "fp16":
            x = x.half()
        tag = args.tag or f"{args.torch_name}-torch-eager-{args.dtype}-b{args.batch}"
